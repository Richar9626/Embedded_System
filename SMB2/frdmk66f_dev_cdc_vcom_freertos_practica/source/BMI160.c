...
{
	/* Registers and values for BMI configuration*/
	uint8_t buffer_tx[CONFIG_LENGHT] = {0x7E, 0x11}; /* 0x11 goes for normal config accelerometer*/
	uint8_t buffer_tx1[CONFIG_LENGHT]= {0x7E, 0x15}; /* 0x15 for normal giroscope*/
	uint8_t buffer_conf[CONFIG_LENGHT]= {0X40, 0x08}; /* frequency 8g range for accelerometer*/
	uint8_t buffer_conf2[CONFIG_LENGHT]= {0X42, 0x08}; /* frequency 8g range for gyroscope*/
	...
	/* we send all packets via I2C there is a little delay needed*/
 ...
 * basically send a write request for sensor and then a read to store data
 */
...
/* Copy of accelerometer changin address*/
