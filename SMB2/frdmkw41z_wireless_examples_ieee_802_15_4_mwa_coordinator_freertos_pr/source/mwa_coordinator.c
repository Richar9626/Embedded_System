...
node_Associated_t nodeAssociated;
...
{ 
    ...
    while(1)
    {
      ...
      case stateListen:
          ...
          if (ev & gAppEvtMessageFromMLME_c) //data from incoming node
          {
              /* Get the message from MLME */
              if (pMsgIn)
              {      
      ...
      if (ev & gAppEvtMessageFromMCPS_c) //data message
      {      
...
/* we check if it is first time for a device to connect so we add all info*/
    if(0 == nodeAssociated.numberOfNodes)
    {
        nodeAssociated.exttAddress[nodeAssociated.numberOfNodes] = pAssocRes->deviceAddress;
        nodeAssociated.shrtAddress[nodeAssociated.numberOfNodes] = pAssocRes->assocShortAddress;
        nodeAssociated.RxOnWhenIdle[nodeAssociated.numberOfNodes] = pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoRxWhenIdle_c;

		if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoDeviceFfd_c)
		{
			nodeAssociated.DeviceType[nodeAssociated.numberOfNodes] = FFD;
		}
		else
		{
			nodeAssociated.DeviceType[nodeAssociated.numberOfNodes] = RFD;
		}

        nodeAssociated.numberOfNodes ++;
    }
    else
    {
    	/* we iterate to find a known extended address on saved nodes*/
    	nodeFound = FALSE;
    	for(int i =0; i < nodeAssociated.numberOfNodes; i ++)
    	{
    		if(pAssocRes->deviceAddress ==  nodeAssociated.exttAddress[i])
    		{
    			nodeFound = TRUE;
    			pAssocRes->assocShortAddress = nodeAssociated.shrtAddress[i]; /* we change the short address for it to be as before*/
    		}
    	}
    	if(!nodeFound)
    	{
    		pAssocRes->assocShortAddress =  0x0001 + nodeAssociated.numberOfNodes; /* we add a different value to the short address*/
            nodeAssociated.exttAddress[nodeAssociated.numberOfNodes] = pAssocRes->deviceAddress;
            nodeAssociated.shrtAddress[nodeAssociated.numberOfNodes] = pAssocRes->assocShortAddress;
            nodeAssociated.RxOnWhenIdle[nodeAssociated.numberOfNodes] = pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoRxWhenIdle_c;

            /* we determine if it is FFD or RFD, end device will be RFC*/
            if(pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoDeviceFfd_c)
			{
				nodeAssociated.DeviceType[nodeAssociated.numberOfNodes] = FFD;
			}
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
	...
	//Serial_PrintDec(interfaceId, pMsgIn->msgData.dataInd.pMsdu[9]);
	TurnOffLeds();
	switch(pMsgIn->msgData.dataInd.pMsdu[9] - ASCII)
	{
		case 0:
			Led2On();
			break;
    ...
    Serial_Print(interfaceId,"\n\r Source ID: 0x000", gAllowToBlock_d);
    Serial_PrintDec(interfaceId, pMsgIn->msgData.dataInd.srcAddr);
   // Serial_PrintHex(interfaceId,&pMsgIn->msgData.dataInd.srcAddr, 2, gPrtHexNoFormat_c);
